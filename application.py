...
class CatGenApplication:
    ...
    def _on_cat_added_to_db(self, event):
        ...
        try:
            cat = event.data
            if cat and cat.id:
                existing = self.database.get_cat(cat.id)
                if not existing:
    ...
    def get_statistics(self) -> dict:
        ...
        try:
            ...
            try:
                pop_size = self.diversity_analyzer.effective_population_size()
                het_data = self.diversity_analyzer.calculate_heterozygosity()
                
                stats['diversity'] = {
                    'effective_size': pop_size.get('effective_size', 0) if isinstance(pop_size, dict) else 0,
                    'mean_heterozygosity': sum(
                        h['observed'] for h in het_data.values()
                    ) / max(len(het_data), 1) if het_data else 0
                }
            except Exception as e:
                logger.warning(f"Could not calculate diversity metrics: {e}")
                stats['diversity'] = {
                    'effective_size': 0,
            ...
            try:
                ...
                stats['inbreeding'] = {
                    'count': len(inbred),
                    'percentage': len(inbred) / max(len(self.registry), 1) * 100 if len(self.registry) > 0 else 0
                }
            except Exception as e:
                logger.warning(f"Could not calculate inbreeding metrics: {e}")
                stats['inbreeding'] = {
                    'count': 0,
                    'percentage': 0
                }
            ...
            return {
                ...
                'with_parents': 0,
                'generations': {},
                'diversity': {'effective_size': 0, 'mean_heterozygosity': 0},
                'inbreeding': {'count': 0, 'percentage': 0}
            }
